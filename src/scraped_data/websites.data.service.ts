...
export class WebSitesDataService {
    ...
    private async queryFromImport(domain: string): Promise<WebsitesData> {
        ...
        if (response) {
            ...
            websiteRawRecord.searchRequestDetails = this.getDataFromImportGroupFieldArray(pageData.backgroundRequests);
            websiteRawRecord.perfTiming = this.getDataFromImportGroupFieldArray(pageData.perfTiming);
            websiteRawRecord.perfTimingSqlTime = this.getDataFromImportGroupFieldArray(pageData.suiteCommerceTag)?.replace(/sqltime:(\d+)/g, '$1');
